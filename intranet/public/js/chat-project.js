...
class ProjectChat {
    constructor(projectId, currentUserId) {
    ...
    loadMessages() {
        fetch(`/chat/api/messages/project/${this.projectId}`)
            ...
            reader.onload = (e) => {
                previewContent = `
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        ...
                        <span>${file.name}</span>
                        <button type="button" onclick="window.projectChatInstance.hideFilePreview()" style="margin-left: auto; background: #dc3545; color: white; border: none; border-radius: 50%; width: 20px; height: 20px; cursor: pointer;">×</button>
                    </div>
                `;
